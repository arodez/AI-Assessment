...
3. **(10 min) Upload `data/trap_sample.csv`** and check:
   - The 2 malformed rows (missing deadline; `not-a-date`) are **reported**, not silently dropped.
   - `Pending` (capital P) is counted as pending.
   - The boundary row (deadline = review date) is handled consistently and the choice is documented.
   - Overdue list matches a hand count.
